...
# CSS로 페이지 상단 마진을 줄여서 헤더를 위로 올리기
st.markdown("""
    <style>
        .main-title {
            font-size:50px;
...
st.write("타이어 상태를 분석하고 결과를 빠르게 제공해드립니다.")
...
# st.title("타이어 상태 분류 서비스")

# # png, jpg, jpeg 파일만 업로드 돠도록 한다. 
...
#     # 사용자가 버튼을 누르면 Flask에서 mlflow로부터 load한 모델을 가져온다. 
#     if st.button("예측하기"):
#         try:
#             # Flask 서버로 이미지를 POST 요청으로 전송한다. 
#             response = requests.post(
#                 "http://172.31.15.63:5002/predict",
#                 files={"file": uploaded_file.getvalue()}
#             )
            
#             # 서버로부터 예측 결과를 받아 표시한다. 
#             if response.status_code == 200:
#                 prediction = response.json().get("predicted_class")
#                 st.success(f"예측 결과: {prediction}")
#             else:
